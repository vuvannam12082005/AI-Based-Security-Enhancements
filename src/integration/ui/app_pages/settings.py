...
def show():
    ...
    with col2:
        ...
        if sensor_status and sensor_status.get('running'):
            ...
            # Get current state from sensor
            current_auto_detect = sensor_status.get('auto_detect', False)
            current_action = sensor_status.get('auto_action', 'throttle')
            
            # Toggle button
            auto_detect_toggle = st.toggle(
                "Auto-Detect",
                value=current_auto_detect,
                key="auto_detect_toggle",
                help="Toggle real-time ML detection and enforcement"
            )
            
            # Action selector (only show when enabled)
            if auto_detect_toggle:
                auto_action_select = st.selectbox(
                    "Action on Threat",
                    ["throttle", "kill"],
                    index=0 if current_action == "throttle" else 1,
                    key="auto_action_select"
                )
            else:
                auto_action_select = current_action
            
            # Detect changes and call API
            if auto_detect_toggle != current_auto_detect or (auto_detect_toggle and auto_action_select != current_action):
                result = api.set_auto_detect(auto_detect_toggle, auto_action_select)
                if result and result.get('ok'):
                    if auto_detect_toggle:
                        st.success(f"✅ Auto-Detect ENABLED ({auto_action_select})")
                    else:
                        st.warning("⚠️ Auto-Detect DISABLED")
                    st.rerun()
                else:
                    st.error("Failed to update auto-detect")
            else:
                # Show current status
                if auto_detect_toggle:
                    st.success(f"🟢 Auto-Detect: ON - Will {auto_action_select} malicious processes")
                else:
                    st.warning("🔴 Auto-Detect: OFF - Manual detection only")
            ...
            if st.button("🛑 Stop Sensor", type="primary"):
                ...
                if result:
                    ...
                    # Show final stats
                    if result.get('events_scanned'):
                        st.info(f"📊 Final stats: {result.get('events_scanned')} scanned, {result.get('threats_detected')} threats, {result.get('processes_blocked')} blocked")
                    st.rerun()
                else:
            ...
            sample_interval = st.number_input("Sample Interval (s)", min_value=0.1, max_value=60.0, value=0.5, step=0.1)
            ...
            auto_detect = st.checkbox(
                "Enable Auto-Detect", 
                value=True,
                help="Automatically analyze events with ML and take action on threats"
            )
            
            if auto_detect:
                auto_action = st.selectbox(
                    "Auto Action on Threat",
                    ["throttle", "kill"],
                    index=0,
                    help="Action to take when malicious process is detected"
                )
                st.warning(f"⚠️ Will automatically {auto_action} malicious processes!")
            else:
                auto_action = "throttle"
            
            if st.button("▶️ Start Sensor", type="primary"):
                result = api.start_sensor(mode, sample_interval, auto_detect, auto_action)
                if result:
                    msg = f"Sensor started in {mode} mode"
                    if auto_detect:
                        msg += f" with auto-detect ({auto_action})"
                    st.success(msg)
                    st.rerun()
                else:
