...
# Suppress the FutureWarning from plotly/pandas datetime conversion
...
def plot_resource_usage_timeline(events: List[Dict[str, Any]]) -> None:
    ...
    if 'timestamp' in df.columns:
        ...
        # Convert to string format for plotly to avoid FutureWarning
    ...
    with col1:
        ...
        if resource_events['cpu_percent'].notna().any():
            # Use time_str instead of datetime to avoid FutureWarning
            fig_cpu = px.line(
                resource_events, 
                x='time_str', 
                y='cpu_percent',
...
def plot_process_activity(events: List[Dict[str, Any]]) -> None:
    ...
    fig = go.Figure(data=go.Heatmap(
        z=pivot.values,
        x=[str(h) for h in pivot.columns],  # Convert to string to avoid datetime issues
